...
<ParamField path="drift_include_blocks" type="array" default="[]">
  Exact block names. Only projects generated by a listed block run drift detection. If empty, all blocks pass. Evaluated before `drift_include_patterns`. See [Backendless: Scope Drift to Specific Projects](/ce/drift/backendless-scoping-projects).
</ParamField>

<ParamField path="drift_exclude_blocks" type="array" default="[]">
  Exact block names. Projects generated by a listed block are skipped during drift detection. Evaluated after `drift_include_blocks` and before `drift_include_patterns`.
</ParamField>
